...
public class IncomeController {
    ...
    @GetMapping("/current")
    public ResponseEntity<ApiResponse<IncomeResponseDTO>> getCurrentIncome(@AuthenticationPrincipal JWTUserData user) {

        IncomeResponseDTO currentIncome = incomeService.getCurrent(user.userId());
        return ResponseEntity.ok(
                new ApiResponse<>(true, "Current income retrieved successfully", currentIncome)
        );
    }
